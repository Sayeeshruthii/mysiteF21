...
from .models import Category, Product, Client, Order, Photo
...
def upload_photo(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            instance = form.save()
            instance.save()
            return redirect('myapp:display_profile')
    else:
        form = UploadPhoto()
...
def display_profile(request):
    if request.method == 'GET':
        ...
        print(username)
        instance = get_object_or_404(Client, username=username)
        return render(request, 'myapp/display_profile_images.html',{'profile_images': instance})
